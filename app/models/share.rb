...
class Share < ActiveRecord::Base
  ...
  validates :path, :presence => true,
	...
	def self.push_shares
		...
		smbconf = "/tmp/shares-%d.%d" % [$$, rand(99999)]
		s = File.new smbconf, "w"
		s.write(self.samba_conf(domain))
		s.flush
		s.close

		lmhosts = "/tmp/lmhosts-%d.%d" % [$$, rand(99999)]
		l = File.new lmhosts, "w"
...
private
	...
	def self.header_workgroup(domain)
		short_domain = Setting.shares.value_by_name "workgroup"
		debug = Setting.shares.value_by_name('debug') == '1'
